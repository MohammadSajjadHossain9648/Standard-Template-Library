...
/*
    Modifiers:
    1.insert() - Insert element in the unordered_multiset.
    2.insert(iterator position, const g) - Adds a new element ‘g’ at the position pointed by the iterator.
    3.erase(const g) - Removes the value ‘g’ from the unordered_multiset.
    4.erase(iterator position) - Removes the element at the position pointed by the iterator.
    5.clear() - Removes all the elements from the unordered_multiset.
    6.emplace() - This function is used to insert a new element into the unordered_multiset container, only if the element
    to be inserted is unique and does not already exist in the unordered_multiset.
    ...
    element is.
    8.swap() - This function is used to exchange the contents of two unordered_multisets but the unordered_multisets must be of the same type,
